...
class SiteContent extends Content {
    ...
    protected function loadSelfValue($key)
    {
        ...
        $this->{'cache' . $this->loadableKeys[$key] . 'Loadable'}($translations, $key);
     ...
     * @param string $key
     */
    protected function cacheMediaFieldLoadable(array $translations, $key)
    {
     ...
     * @param string $key
     */
    protected function cacheContentRelationFieldLoadable(array $translations, $key)
    {
    	foreach($translations as $l => $translation) {
    		$this->loadables[$key][$l] = empty($translation)
    			? null
     ...
     * @param string $key
     */
    protected function cacheTextEditorFieldLoadable(array $translations, $key)
    {
    	foreach($translations as $l => $translation) {
        		$this->loadables[$key][$l] = empty($translation)
        			? null
    ...
    protected function loadExtensionValue($key)
    {
    	...
    	$this->{'cache' . $extension->type . 'Loadable'}($translations, $key);
